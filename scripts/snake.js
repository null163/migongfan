...
function resize() {
  ...
  maskLeft.style.left = 7.5 / 659 * windowHeight + 'px'
  ...
  head.style.left = Left - 22 / 659 * windowHeight + 'px'
...
function drawGame() { //打印贴图
  ...
  if (!gameOn && !gameOver) {
    ...
    div.style.top = (snake[0].y * cellSize - 12) / 659 * windowHeight + 'px'
    ...
    if (gameOver) {
      ...
      head.style.width = (cellSize + 9) / 659 * windowHeight + 'px'
      head.style.height = (cellSize + 17) / 659 * windowHeight + 'px'
    }
  }
  else if (snake[0].dirX === 0 && snake[0].dirY === -1 || (gameOver || settle) && snake.length > 1 &&
    (snake[0].dirX === 1 && snake[0].dirY === -1 && snake[1].x === snake[0].x + 1 || snake[0].dirX === -1 &&
